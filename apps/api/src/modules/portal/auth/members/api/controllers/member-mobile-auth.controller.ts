...
import { CurrentMember } from "@modules/portal/auth/members/api/decorators/current-member.decorator";
import { RequireMemberJwtMobile } from "@modules/portal/auth/members/api/decorators/require-member-jwt.decorator";
...
export class MemberMobileAuthController {
    ...
    async getMe(@CurrentMember() member: Member): Promise<MemberMeResponseDto> {
        ...
        const list = this.membersService.toCrmMemberListDto(fullMember);
        return {
            id: fullMember.user.id,
            email: fullMember.user.email,
            name: list.name,
            phone: list.phone || undefined,
