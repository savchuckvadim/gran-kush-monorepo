...
import { SESSION_INCLUDE } from "@modules/portal/crm/presence/infrastructure/prisma-includes";
...
export class PresenceSessionPrismaRepository extends PresenceSessionRepository {
    ...
    private mapToEntity(
        row: Prisma.PresenceSessionGetPayload<{ include: typeof SESSION_INCLUDE }>
    ): PresenceSession {
        let memberPayload: PresenceSession["member"];
        ...
        return new PresenceSession({
            id: row.id,
            memberId: m?.id ?? row.entityRecordId,
